...
def thread_getOneBook(indexUrl):
    ...
    # 获取这本书的所有章节
    charts_url = []
    indexUrl = 'http://www.shuanshu.com'
    # 提取出书的每章节不变的url
    ...
    num = 1
    for j in charts_url:
        p.apply_async(get_ChartTxt, args=(j, title, num))
        num += 1
...
def process_getAllBook(base):
    ...
    book_indexUrl = [
        'http://www.shuanshu.com/files/article/html/6/6231/index.html'
    ]
    print("-------------------开始下载-------------------")
    p = []
    p.append(multiprocessing.Process(target=thread_getOneBook, args=(book_indexUrl)))
    print("等待所有的主进程加载完成........")
    for i in p:
        i.start()
    for i in p:
...
if __name__ == "__main__":
    # # 主页
    # base = 'http://www.shuanshu.com'
    # 下载指定的书
    # process_getAllBook(base)
    # 如果下载完出现卡的话，请单独执行如下命令

    sort_allCharts(r'.\驭灵女盗01', "驭灵女盗完整版.txt")
